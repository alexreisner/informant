...
Informant is a full-featured form builder for Ruby on Rails which promotes a simple syntax that keeps your views clean. The goal is for everything about a field (label, description, error display, etc) to be encapsulated in a single method call. Examples and general information are below; please read the {API documentation}[http://rdoc.info/projects/alexreisner/informant] for more complete/detailed information.
...
renders HTML like this (assuming the <tt>@car</tt> is a new record):
  ...
  <form action="/cars" class="new_car" id="new_car" method="post">
    ...
    <div class="submit">  
      <input id="car_submit" name="commit" value="Create" type="submit" />
    </div>
